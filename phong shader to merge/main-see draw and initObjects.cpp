...
void draw()
{
   ...
   g_meshShader->use();
   g_meshShader->setProjMatrix(g_proj);
   g_meshShader->setViewMatrix(g_view);

   g_meshShader->setLight(g_lightInfo);
   g_meshShader->setCamPos(vec3(0));


   // TODO - set lights for shader

   
   for (int i = 0; i < (int)drawable_objects.size(); i++) {
      drawable_objects[i]->draw(g_meshShader, g_model_trans);
   }
...
void initObjects() {
   ...
   g_meshShader = new PhongShader();

   // Bunnie
   vec3 pos(0,0,5);
   vec3 vel(0, 0, 0.1);

   GameDrawableObject *object = new GameDrawableObject("models/cube.m");
   object->setPosition(vec3(0, 0, 3));
   object->setScale(vec3(1));
   drawable_objects.push_back(object);


   // *** light ***
   // TODO - Port to use GameLight object. Pretty much the Same thing.
