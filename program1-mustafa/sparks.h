/*
 init and rendering from GLSL 4.0 cookbook chapter 9
...
- use a pointer object.
  particles = new Sparks();
  
- call init after creating the object
  particles->init(pos0); // pos0 is initial position in world coords


- on update():
  particles->update(t, dt); // float time in seconds
...
- on every draw:
    ...
    // setModel();
    particles->setModelMatrix(mat4(1.0f)); // other model matrices make things complicated
    particles->setPos(newPos); // optional
    particles->render();
...
class Sparks : public Drawable {
    public:
        int id;

        Sparks();
        Sparks(float time);
        Sparks(float time, float spread, float iVel);
