...
Sparks::Sparks(float time, float spread, float iVel) {
    t = time;
    m_spread = spread;
    m_iVel = iVel;
}
...
bool Sparks::init(vec3 p0) {
    ...
    setUniform("Accel", vec3(0.0f, -5.0f,0.0f));
...
bool Sparks::initBuffers() {
    ...
    float time = 0.0f;
    float rate = 0.9 * life/(nParticles);
    for(int i = 0; i < nParticles; i++ ) {
        data[i] = time;
        time += rate;
    }
...
void Sparks::setViewMatrix(mat4 mat) {
    GLint prog = m_shaderProg;
    glUseProgram(prog);
...
void Sparks::setProjMatrix(mat4 mat) {
    GLint prog = m_shaderProg;
    glUseProgram(prog);
...
bool Sparks::installTFShader() {

    char *vShaderFName = (char *) "sparks_vert.glsl";
    char *fShaderFName = (char *) "sparks_frag.glsl";
